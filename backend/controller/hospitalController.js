...
const getDonationHistory = async (req, res) => {
    try {
        ...
        // Validate the donorId format
        if (!mongoose.Types.ObjectId.isValid(donorId)) {
            return res.status(400).json({ message: 'Invalid donor ID format' });
        }

        // Fetch the donor by ID and select donationHistory and role
        const donor = await User.findById(donorId).select('donationHistory role').populate({
            path: 'donationHistory.hospitalId',
            model: 'User',
            select: 'name -_id'
        });
        ...
        res.status(200).json({ donationHistory: donor.donationHistory });
    } catch (error) {
...
module.exports = { addUser, removeUser, updateDonationHistory, getDonationHistory}
